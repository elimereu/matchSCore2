---
title: >
  Comparing scRNA-seq data with `matchSCore2`.
author:
...
1. The reference dataset is subsetted in two parts, train and test data, in order to estimate the accuracy of the model.
...
![](overview_package.png){width=800%} 
...
While with the previous approach we can fastly transfer annotations from one dataset to another, sometimes we are interested in comparing gene measurements between cells coming from different experiments (e.g. WT vs KO, disease vs control, different protocols, etc..). In the matchSCore2 package, we provide a mathematical framework to align datasets, by projecting them to the same coordinate space of an assigned reference dataset. The method is based on the single value decomposition and allows a direct comparison of datasets, by finding the optimal linear transformation between data points.     
...
In this example, we want to align 3 of the PBMC+HEK293T experiments sequenced with different protocols: Chromium, Quartz-Seq2 and Smart-Seq2. In this case, the choice of the reference is not so important, we will choose the Chromium dataset. The method requires the clusters (more than 1 cell), they will be used to compute a centroid point at each dataset, useful for their comparison. 
```{r, eval=FALSE}
...
quartzseq <- scater::filter(sce,batch=="Quartz-Seq2")

chromium$cluster <- factor(chromium$ident)
smartseq2$cluster <- factor(smartseq2$ident)
quartzseq$cluster <- factor(quartzseq$ident)


list_data <- list(Chromium=chromium,Smartseq2=smartseq2,Quartz.seq=quartzseq)

markers.qs <- markers$`Quartz-Seq2`
gene_cl.qs <- cut_markers(levels(markers.qs$cluster),markers = markers.qs,ntop = 100)

markers_list <- unique(c(unlist(gene_cl.ref),unlist(gene_cl.test),unlist(gene_cl.qs)))
...
The output contains a `SingleCellExperiment` object with the joint raw count and integrated matrixes from the 3 datasets. See the manual of the `align_run` function for more details about the output.

Once data are integrated, you can use the normalized matrix to cluster your cells. For example, if you want to use Seurat to cluster your cells, you can provide that matrix in the @data slot, as in the `seurat3_run` function. 
