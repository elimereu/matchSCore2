...
```{r eval=FALSE}
# TODO - train_model should have a `verbose` element to prevent printing of unessential messages 
mod <- train_model(
...
matchSCore2(gene_cl.ref = gene_cl.ref, 
...
By using the function `align_run`, two or more gene expression matrices are compared and aligned to the same reference dataset.  
...
Given that a centroid is computed for each dataset, by combining the gene measurements from each cluster, the method requires that clusters have more than 1 cells.
