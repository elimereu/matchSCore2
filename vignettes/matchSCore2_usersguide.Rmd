...
download.file(
  url = url,
  destfile = "~/Downloads/sce.all_classified.technologies.RData",
  mode = "wb"
)
```
...
nnet2 <- ref$nnet2
smartseq2 <- readRDS("datalist_smartseq2.rds")
```
...
gene_cl.test <- ms_top_markers(levels(test.markers$cluster),
...
In this case, the choice of the reference is not so important, thus we will choose the Chromium dataset.
...
download.file(
  url = url,
  destfile = "sce.all_classified.technologies.RData",
  mode = "wb"
)
```
