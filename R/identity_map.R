...
#'
#' This function infers cell identities by using the model learned by the
#' reference dataset.
#'
...
#' Default=0.5.
#'
#' @return A list with a vector with cell identities predicted by the model (ids)
#' and a dataframe with probabilities at each identity class.
#'
#' @export
#'
#' @examples
...
identity_map <- function(scale.data,
  ...
  # require(Matrix)
  p <- lapply(gene_cl.ref,
             function(x) rownames(scale.data)[which(rownames(scale.data) %in% x)])

  var.test <- sapply(p,
                     function(x) Matrix::colSums(scale.data[x, ]))
  ...
  if (is.null(p.threshold))
    p.threshold <- 0.5
  fit <- apply(fitted.results, 1, function(x)
    ifelse(max(x) > p.threshold, colnames(fitted.results)[which(x == max(x))], "unclassified"))
