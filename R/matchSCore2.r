...
#' @param gene_cl.ref A list of reference group markers. Each element of the list represents the set of markers for each biological group.
#' @param gene_cl.obs A list of cluster-specific gene sets to match with gene_cl.ref.
#' @param ylab A character label (ylab) indicating the reference technology to plot in the matchSCore table(e.g. "Smart-Seq2").
#' @param xlab A character label (xlab) indicating the tested technology to plot in the matchSCore table (e.g. "Chromium").
#' @return A list with the matchSCore matrix and the ggplot table at the ggplot slot.
#' @export
#' @examples
#'
...
matchSCore2 <- function(gene_cl.ref,gene_cl.obs,ylab,xlab){
  ...
  #list(matchScore=score,labels=anno.lab,max_JI=max_ji,JI.mat=ji_mat,ggplot=gg)
  return(gg)
