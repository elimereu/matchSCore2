...
align_run <- function(dataset_list,marker_list,ref){
  ...
  #d_list <- lapply(d_list,function(x) t(scale(t(x),center = T,scale = apply(x, 1, sd, na.rm = TRUE))))
  ...
  # if(length(to_rm)>1){
  #     print(paste(" ",to_rm,"cluster has limitated size"))
  #     stop("Try to combine them with other clusters")
  # }
  # # 
  # cl1 <- levels(factor(unlist(cl)))
  # cl1 <- cl1[which(!(cl1 %in% to_rm))]
  # levels(cl[[x]])[!which(cl[[x]] %in% to_rm[[x]])]
  # cl1 <- cl[[1]]
  # cl1[which(cl1 %in% to_rm)] <- NA
  # cl1 <- factor(cl1)
  # 
  # cl1 <- lapply(c(1:length(to_rm)),function(x) cl[[x]][which(cl[[x]] %in% to_rm[[x]])])
  # 
  cl1 <- cl
  ...
  # sapply(levels(cl[[x]])[which(to_rm[[x]] %in% levels(cl[[x]]))],function(y) print(y))
  # lapply(seq(1:length(d_list)),function(x) sapply(levels(cl[[x]])[which(to_rm[[x]] %in% levels(cl[[x]]))],function(y) print(y) ))
