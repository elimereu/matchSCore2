...
train_model <- function(scale.data,
  ...
  start.time <- Sys.time()
  sizes <- unlist(lapply(levels(clus),
                         function(x) length(clus[which(clus %in% x)])))
  ...
  train.sample <- unlist(sapply(levels(clus),
                                function(x) sample(x = which(clus %in% x), size = sizes[names(sizes) == x] * prop)))
  ...
  fit <- apply(fitted.results, 1,
               function(x) colnames(fitted.results)[which(x == max(x))])
  if (is.null(p.threshold)) {
    p.threshold <- 0.65
  }
  fit <- apply(fitted.results, 1,
               function(x) ifelse(max(x) > p.threshold,
                                  colnames(fitted.results)[which(x == max(x))],
                                  "unclassified"))
  ...
  print(table(id_test = fit_res$out.test,
              class = fit_res$fit))
