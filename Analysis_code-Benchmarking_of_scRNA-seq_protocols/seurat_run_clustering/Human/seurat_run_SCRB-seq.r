...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "SCRB-Seq")
dim(data@data)
#24109  1713
...
data <-  FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.3, x.high.cutoff = 2.5, y.cutoff = 0.5)
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:10, resolution = 0.3, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0   1   2   3   4 
685 376 271 224 157
...
levels(data@ident) <- c("Cytotoxic T cells","CD14+ Monocytes","NK and Cytotoxic T cells","B cells","HEK cells")
