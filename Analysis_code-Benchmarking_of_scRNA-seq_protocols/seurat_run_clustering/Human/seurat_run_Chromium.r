...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "Chromium")
dim(data@data)
##27818  1634
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.1, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:12, resolution = 0.7, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0   1   2   3   4   5   6   7   8 
372 371 263 156 141 116 105  72  38 

data <- RunTSNE(object = data, dims.use = 1:12, do.fast = TRUE,force.recalc=T)
...
levels(data@ident) <- c("CD4 T cells 1","HEK cells 1","CD14 Monocytes","CD4 T cells 2","CD8 T cells","NK cells","B cells","HEK cells 2","FCGR3A Monocytes")
