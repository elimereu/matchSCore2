...
#29.0   884.2  1345.5  1462.5  1914.8  9413.0
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.2, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:13, resolution = 0.3, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0    1    2    3    4    5    6 
1310  626  483  307  237   64   31 

data <- RunTSNE(object = data, dims.use = 1:13, do.fast = TRUE,force.recalc=T)
data <- RunUMAP(object = data,dims.use = 1:13)
library(ggplot2)
...
png(file="out_Seurat/plots/TSNE_clusters_annotated.png",width = 6, height = 6,units = 'in', res = 300)
tsne
dev.off()
...
FeaturePlot(object = data, features.plot = c("Polr1b","Slc26a3", "Ceacam1","Reg4","Muc2","Agr2","Pcna"), cols.use = c("lightgray", "blue"), 
            reduction.use = "tsne")
dev.off()
...
png(file="out_Seurat/plots/TSNE_Known_markers2.png",width = 12, height = 12, units = 'in', res = 600)
FeaturePlot(object = data, features.plot = c("Slc26a3", "Ceacam1","Lgr5","Smoc2","Reg4","Muc2","Tff3","Mki67","Pcna","Lamc2","Fabp2"), cols.use = c("lightgray", "blue"), 
            reduction.use = "tsne")
dev.off()
