...
#135.0   570.0   794.5  1053.5  1211.2  8230.0 
...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "Dropseq")
rm(cd.sparse)
dim(data@data)
#11610   288
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.38, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:13, resolution = 1, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0  1  2  3  4  5 
61 57 53 45 42 15 
