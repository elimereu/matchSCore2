...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "CEL-Seq2")
rm(cd.sparse)
dim(data@data)
#20803   180
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.02, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:13, resolution = 1.4, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0  1  2  3  4 
46 46 34 31 23 
