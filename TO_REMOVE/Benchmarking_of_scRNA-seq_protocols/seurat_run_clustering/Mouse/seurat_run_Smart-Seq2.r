...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "Smart-Seq2")
rm(cd.sparse)
dim(data@data)
#16122   303
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.11, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:14, resolution = 0.8, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0   1   2   3   4 
117  73  53  38  22 
...
rownames(data@data)[grep("Neurog",rownames(data@data))]
