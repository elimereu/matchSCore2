...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "Quartz-Seq2")
dim(data@data)
#28666  1388
...
data <-  FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.012, x.high.cutoff = 3.5, y.cutoff = 0.5)
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:10, resolution = 0.4, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0   1   2   3   4   5   6 
397 380 187 182 125  62  55 
...
levels(data@ident) <- c("CD14+ Monocytes","HEK cells","NK and Cytotoxic T cells","Cytotoxic T cells 1","Cytotoxic T cells 2","B cells","FCGR3A+ Monocytes")
