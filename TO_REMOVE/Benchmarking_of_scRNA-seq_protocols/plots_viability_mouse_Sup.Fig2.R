load(file="10XV3Rev.mmus.full.SCE.20Kcells.Robj")

library(scater)

sce <-filter(full.SCE.mmus,Species=="Mouse")
load(file="mito_ENS.id_Mouse.RData")

sce <- calculateQCMetrics(sce, feature_controls = list(Mt = which(rownames(sce) %in% mito_ENS)))
...
table(Library)
Library
AN4491 AN4493 AN4492 
7997   1459    869


load(file="ChromV3_thresholds/ChromV3_EmptyDroplets_selected_cells_human_and_mouse.RData")
...
lib <- pd_new$Library
# table(pd_new$Mapped[which(lib=="Without.Viability")]>10000)
# FALSE  TRUE 
# 6175  1795 
# > 
...
png(file="nGenesVS_MTcontent_Mouse.png",width = 8, height = 6, units = 'in', res = 600)
ggplot(pd_new,aes(x=nGenes,y=pct_counts_Mt))+geom_jitter(aes(colour=Library,shape=low_quality),alpha=0.3,size=1.5)+theme_bw()+scale_color_manual(values = c("red","blue","green4"))+
...
png(file="Mapped_vs_Ngenes_20K_shape_by_highMito_Mouse_NEW.png",width = 8, height = 6, units = 'in', res = 600)
ggplot(pd_new,aes(x=Mapped,y=nGenes))+geom_point(aes(colour = Library,shape=low_map_cell),size=1.5,alpha=0.5)+geom_vline(xintercept = c(1778),linetype="dotted")+theme_bw()+scale_color_manual(values = rev(c("blue","red")))+
