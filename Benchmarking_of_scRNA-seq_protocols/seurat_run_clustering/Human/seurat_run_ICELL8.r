...
data <- CreateSeuratObject(raw.data = cd.sparse, min.cells = 5, min.genes = q, project = "ICELL8")
dim(data@data)
#25861  2012
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:9, resolution = 0.5, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0   1   2   3   4   5 
647 420 403 341 163  38  
...
levels(data@ident) <- c("Cytotoxic T cells","NK and Cytotoxic T cells","CD14+ and FCGR3A+ Monocytes","HEK cells","B cells","unknown")