...
#63.0   382.5   641.0   813.1  1181.5  2663.0 
...
data <- FindVariableGenes(object = data, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.6, 
...
data <- FindClusters(object = data, reduction.type = "pca", dims.use = 1:8, resolution = 1, print.output = 0, save.SNN = TRUE,force.recalc=TRUE)
table(data@ident)
0  1  2  3 
32 30 19  9 

data <- RunTSNE(object = data, dims.use = 1:8, do.fast = TRUE,force.recalc=T,perplexity=20)
...
png(file="out_Seurat/plots/TSNE_Known_markers2.png",width = 12, height = 12, units = 'in', res = 600)
FeaturePlot(object = data, features.plot = c("Polr1a","Polr1b","Slc26a3", "Ceacam1","Lgr5","Smoc2","Reg4","Atoh1","Muc2","Tff3","Mki67","Foxm1","Pcna","Lamc2","Fabp2"), cols.use = c("lightgray", "blue"), 
            reduction.use = "tsne")
dev.off()
